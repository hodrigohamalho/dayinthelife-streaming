...
Now choose the `Users Database` connection created previously.
...
Click now on `Save`.
Please pay attention not to publish! Otherwise, you will have to wait for the publishing process to finish.
...
Now, we need to wait Openshift build and deploy our container. When done, you should see `Published version 1` on the top of the page.
...
If you put exactly the same name suggested here, the url will be:
